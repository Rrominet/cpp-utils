...
namespace ml
{
    class Agent
    {
        public:
            ...
            void addBeforeLlmCall(const std::function<std::string(const std::string&)>& f) {_beforeLlmCall.push_back(f);}
            ...
            void addAfterLlmCall(const std::function<std::string(const std::string&, const std::string&)>& f) {_afterLlmCall.push_back(f);}
        ...
        private:
            ...
            //the string argument is the _inData;
            //the return string will be add if not empty in the _inData sended to the llm.
            ml::Vec<std::function<std::string(const std::string&)>> _beforeLlmCall;

            //the 1st argument is the in data sent to the llm;
            //the 2nd argument is the data 
            //the return string is the data that gonna be available in _outData by other Agents.
            ml::Vec<std::function<std::string(const std::string&, const std::string&)>> _afterLlmCall;
